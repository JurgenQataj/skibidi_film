...
import styles from "./ActorAgeGamePage.module.css";
import { FaTrophy, FaMedal, FaArrowLeft, FaBirthdayCake } from "react-icons/fa";
import { jwtDecode } from "jwt-decode";

const API_URL = import.meta.env.VITE_API_URL || "";

function ActorAgeGamePage() {
  ...
  useEffect(() => {
    if (!token) return;
    axios
      .get(`${API_URL}/api/actor-age-game/my-score`, {
  ...
  useEffect(() => {
    setLbLoading(true);
    axios
      .get(`${API_URL}/api/actor-age-game/leaderboard`)
  ...
  return (
    <div className={styles.page}>
        ...
        <div className={styles.titleRow}>
          ...
          <div className={styles.titleContent}>
            <h1 className={styles.title}>
              Indovina il <span className={styles.titleHighlight}>Giovane</span>
            </h1>
            <p className={styles.subtitle}>Chi dei due attori è più giovane?</p>
          </div>
        </div>
      </div>
      ...
      <div className={styles.howToPlay}>
        <div className={styles.step}><span className={styles.stepNum}>1</span><span>Due attori famosi appaiono fianco a fianco</span></div>
        <div className={styles.step}><span className={styles.stepNum}>2</span><span>Clicca su chi pensi sia nato dopo (più giovane)</span></div>
        <div className={styles.step}><span className={styles.stepNum}>3</span><span>Più risposte giuste = streak più alto!</span></div>
      </div>
      ...
      <div className={styles.playSection}>
        <button
          ...
          onClick={() => navigate("/actor-age-game/play")}
        >
          <div className={styles.playBtnIcon}><FaBirthdayCake /></div>
          <div className={styles.playBtnText}>
            <span className={styles.playBtnTitle}>Gioca!</span>
            <span className={styles.playBtnDesc}>Chi è il più giovane?</span>
          </div>
...
export default ActorAgeGamePage;
